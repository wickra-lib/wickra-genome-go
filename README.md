...
[![Go module](https://raw.githubusercontent.com/wickra-lib/.github/main/profile/badges/go.svg)](https://pkg.go.dev/github.com/wickra-lib/wickra-genome-go)
...
**Go bindings for the Wickra Genome vector engine over its C ABI hub (cgo). A `Genome` is built from a spec JSON and driven with command JSONs, so the same commands yield the byte-identical similarity, clustering and anomaly results as every other Wickra Genome binding.**
...
directory of [wickra-genome](https://github.com/wickra-lib/wickra-genome) by the release
pipeline: it mirrors the Go sources, the vendored C ABI header (`include/wickra_genome.h`)
...
Wickra Genome is research and analytics software. Its outputs are
